...
const double beam_current = 10.000;   // 10A ions
...
const double beam_current_e = 10.000; // 10A electrons
...
double h = 0.0003;
double cathodepot = -25000.0;             // -25kV cathode
double anodepot = 0.0;
...
double uniform01(){ return u01(gen); }

// Volumetric uniform sampling within a sphere of radius rad (full ball,
// including the core - the central region inside the cathode shell).
// Mimics gas-discharge ionisation throughout the chamber volume.
// Rejection sampling on the bounding cube keeps density uniform per
// unit volume. Particles in mesh-solid nodes are rejected.
ParticleP3D sample_volume(double rad, const Geometry &geom) {
    while (true) {
        double x_f = (2.0 * uniform01() - 1.0) * rad;
        double y_f = (2.0 * uniform01() - 1.0) * rad;
        double z_f = (2.0 * uniform01() - 1.0) * rad;
        if (x_f*x_f + y_f*y_f + z_f*z_f > rad*rad) continue;
        ...
        if (geom.mesh(i, j, k) == 0){
            return ParticleP3D(0, x_f, 0.0, y_f, 0.0, z_f, 0.0); // at rest
        }
    }
}

// Surface sampling on a sphere of radius rad - kept for reference.
...
// Both species sample volumetrically within the full anode-radius sphere,
// INCLUDING the core (r < cathode_r). This produces a population of
// slow particles distributed throughout the chamber - matching the
// gas-discharge ionisation source of a real IEC fusor.
void add_ions(ParticleDataBase3D &pdb, Geometry &geom) {
    for (long i = 1; i <= N_clouds; i++ ){
        pdb.add_particle(IQ, 1, m, sample_volume(anode_r, geom));
    }
}

void add_electrons(ParticleDataBase3D &pdb_e, Geometry &geom) {
    for (long i = 1; i <= N_clouds_e; i++) {
        pdb_e.add_particle(IQ_e, -1, m_e, sample_volume(anode_r, geom));
    }
}
...
void sim(int argc, char **argv){
    ...
    // Under-relaxation: blend new solve with previous iter's epot
    // (epot_relaxed = alpha * epot_new + (1-alpha) * epot_old) to
    // damp bistable oscillation in highly non-linear space-charge regimes.
    ...
    while (j <= 5) {
        ...
        if (j > 1) {
            int nc = epot.nodecount();
            for (int a = 0; a < nc; a++) {
                epot(a) = alpha * epot(a) + (1.0 - alpha) * epot_old(a);
            }
        }
        efield.recalculate();
        scharge.clear();
    ...
    // Space-charge density map (final iter scharge)
    gplotter.set_scharge(&scharge);
    gplotter.set_fieldgraph_plot(FIELD_SCHARGE);

    gplotter.set_view(VIEW_XY, -1);
    gplotter.set_ranges(-0.05, -0.05, 0.05, 0.05);
    gplotter.plot_png((run + "scharge_xy.png").c_str());

    gplotter.set_view(VIEW_XZ, -1);
    gplotter.set_ranges(-0.05, -0.05, 0.05, 0.05);
    gplotter.plot_png((run + "scharge_xz.png").c_str());

    gplotter.set_view(VIEW_YZ, 105);
    gplotter.set_ranges(-0.05, -0.05, 0.05, 0.05);
    gplotter.plot_png((run + "scharge_yz_x0.png").c_str());

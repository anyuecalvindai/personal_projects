 ...
 * to run_no_anode/ for direct comparison against the standard geom.dat
 * (tee + cathode + anode) under identical solver settings.
 *
 * Output files (run_no_anode/):
 *   geom_cat_only.dat
 *   epot_xy_no_anode.png
 *   epot_xz_no_anode.png
 *   epot_yz_x0_no_anode.png
 *   potential_radial_x_no_anode.dat
 *   potential_radial_y_no_anode.dat
 *   potential_radial_z_no_anode.dat
 */
#include <cstdlib>
#include <sstream>
#include <fstream>
...
void sim(int argc, char **argv){
    string run = "run_no_anode/";

    // --- Build the no-anode geometry ---
    ...
    geom.set_solid(8, s2);
    // No anode solid - this is the whole point of this run.
    ...
    // No boundary 9 (anode) - completely absent.

    geom.build_mesh();
    geom.build_surface();

    // Save the no-anode mesh for re-use / inspection
    geom.save("geom_cat_only.dat", true);
    ...
    gplotter.plot_png((run + "epot_xy_no_anode.png").c_str());

    gplotter.set_view(VIEW_XZ, -1);
    gplotter.set_ranges(-0.05, -0.05, 0.05, 0.05);
    gplotter.plot_png((run + "epot_xz_no_anode.png").c_str());

    gplotter.set_view(VIEW_YZ, 105);
    gplotter.set_ranges(-0.05, -0.05, 0.05, 0.05);
    gplotter.plot_png((run + "epot_yz_x0_no_anode.png").c_str());

    // --- Radial potential profiles along each axis ---
    {
        ofstream opot_z(run + "potential_radial_z_no_anode.dat");
    ...
    {
        ofstream opot_y(run + "potential_radial_y_no_anode.dat");
    ...
    {
        ofstream opot_x(run + "potential_radial_x_no_anode.dat");
